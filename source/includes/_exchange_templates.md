...
All exchange template code snippets must export a function which takes in a [context object](#exchange-template-code-context-object) and returns a [token object](#exchange-template-code-response-object).

### Exchange Template Code Context Object

Attribute | Type | Description
--------- | ---- | -----------
`sourceToken` | *object* | The source token that matches the `source_token_type` of the [exchange template object](#exchange-template-object)
`configuration` | *array* | The configuration defined for the [exchange object](#exchange-object)

### Exchange Template Code Response Object
...
Returns an [exchange template object](#exchange-template-object) if the exchange template was created. Returns [an error](#errors) if there were validation errors or the exchange template failed to create.
...
Returns a [paginated object](#pagination) with the `data` property containing an array of [exchange template objects](#exchange-template-object). Providing any query parameters will filter the results. Returns [an error](#errors) if exchange templates could not be retrieved.
...
var application = await client.GetByIdAsync(new Guid("17069df1-80f4-439e-86a7-4121863e4678"));
```
...
Returns an [exchange template object](#exchange-template-object) with the `id` provided. Returns [an error](#errors) if the exchange template could not be retrieved.
...
var exchangeTemplate = await client.UpdateAsync(new Guid("17069df1-80f4-439e-86a7-4121863e4678"), 
...
Returns an [exchange template object](#exchange-template-object) if the exchange template was updated. Returns [an error](#errors) if there were validation errors or the exchange template failed to update.
...
await client.DeleteAsync(new Guid("17069df1-80f4-439e-86a7-4121863e4678"));
```
